...
SRR (Systems–Roles Repository): a systems×roles (3×3) project repository template aligned with FPF

**S2R** — имя метода.
**SRR** — его формальная расшифровка: Systems–Roles Repository (Хранилище систем и ролей).
Этот репозиторий — **шаблон (Template)** структуры SRR: матрица 3×3 «Системы × Роли» + управляющее семейство.
...
## Для чего S2R (Job to be Done)

S2R нужен, когда в проекте появляется типичная боль:
...
**S2R превращает хранилище в контролируемую модель проекта**, где каждый документ имеет «координаты» (какая система × для какой роли).
...
SRR (Systems–Roles Repository) — это способ организации знаний через **матрицу 3×3**, которая определяет **9 семейств документов** + **1 управляющее семейство** (F0).

### Матрица 3×3: 9 семейств документов
...
### F0: Управляющее семейство

**F0 — это 10-е семейство**, которое управляет остальными девятью. В отличие от F1-F9, управляющее семейство **не делится по ролям** Предприниматель/Инженер/Менеджер, а организовано по функциональным областям:
...
> **Важно:** Названия должны быть осмысленными и отражать суть вашего проекта.
...
3. Покажи текущее покрытие матрицы SRR (9 семейств)
```
...
│
└── content/                     # Контент по SRR (10 семейств)
    ├── 0.Management/            # F0: Управляющее семейство (не делится по ролям)
...
- **Введение в системное мышление:** [docs.system-school.ru](https://docs.system-school.ru/ru/personal/systems-thinking-introduction/preface) — обязательное чтение для понимания S2R
